...
function createProfilePayloads(data, type) {
  ...
  if (type === 'name') {
    ...
    payloads.push({
      worksFor: [{
        '@type': 'Organization',
        url: blockchainIdUrl(data.employer, 'id')
      }]
    })
  } else if (type === 'avatar') {
    payloads.push({
    ...
    data.knows.split(', ').map(function(username) {
      payloads.push({
        knows: [{
          '@type': 'Person',
          url: blockchainIdUrl(username, 'id')
        }]
      })
    })
  } else {
...
var createTokens = function(inputData, rawPrivateKey, rawPublicKey) {
  ...
  var tokenTypes = [
    'name', 'bio', 'employer', 'avatar', 'website',
    'facebook', 'twitter', 'linkedin', 'github',
    'bitcoin', 'birthDate', 'address', 'knows'
  ]

  var subject = {
    '@type': 'Person',
    url: blockchainIdUrl(inputData.username, 'id')
  }
